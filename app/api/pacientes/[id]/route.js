...
export async function GET(request, { params }) {
  try {
    ...
    console.error('Error proxying paciente GET:', err?.message || err);
...
export async function PUT(request, { params }) {
  try {
    ...
    console.error('Error proxying paciente PUT:', err?.message || err);
    return NextResponse.json({ success: false, error: 'Error del servidor', details: err?.message }, { status: 500 });
  }
}
...
export async function DELETE(request, { params }) {
  try {
    ...
    console.error('Error proxying paciente DELETE:', err?.message || err);
    return NextResponse.json({ success: false, error: 'Error del servidor', details: err?.message }, { status: 500 });
  }
}
