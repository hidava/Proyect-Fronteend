...
export async function POST(request) {
  try {
    ...
    console.log('[auth/v1/register POST] EXTERNAL_API:', EXTERNAL_API);
    
    if (!EXTERNAL_API) {
      console.error('[auth/v1/register POST] EXTERNAL_API_BASE no configurado');
    ...
    console.log('[auth/v1/register POST] Calling:', apiUrl);
    ...
    console.log('[auth/v1/register POST] Response status:', res.status);
    
    // Verificar si la respuesta es JSON válido
    const contentType = res.headers.get('content-type');
    if (!contentType || !contentType.includes('application/json')) {
      const text = await res.text();
      console.error('[auth/v1/register POST] API no devolvió JSON:', text.substring(0, 200));
